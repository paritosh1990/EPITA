    ...
    Snap.plugin(function (Snap, Element, Paper, glob) {
        ...
        function equaliseTransform(t1, t2, getBBox) {
            ...
            for (; i < maxlength; i++) {
                ...
                from[i] = [];
                to[i] = [];
                for (j = 0, jj = Math.max(tt1.length, tt2.length);
                j < jj;
                j++) {
...
// jquery.animatedPercentageGraph.min.js
(function(b, l, m, n) {
    var d = function(a, c) {
        this.el = a;
        this.$el = b(a);
        this.options = c;
        this.metadata = this.$el.data("apg-options")
    };
    d.prototype = {
        defaults: {
            ringColor: "#e1e1e1",
        ...
        init: function() {
            this.config = b.extend({}, this.defaults, this.options, this.metadata);
            var a = this.$el.data("percentage");
            this.config.originalWasPercentage = /%$/.test(a);
            this.config.originalWasPercentage || (this.config.displayFactor = Math.pow(10, Math.round(Math.log(a) / Math.log(10))),
            a = 100 * a / this.config.displayFactor);
            a = parseFloat(a);
            this.config.originalValueWasInt = 0 === a %1;
            this.percentage = a / 100;
            this.$svg = b('<svg class="apg-svg" />').appendTo(this.$el);
            this.$label = b('<div class="animated-percentage-graph-number-display"/>').insertAfter(this.$el);
            this.$label.css("color", this.config.arcColor);
            this.$label.html('0<span class="percent-sign">%</span>');
            this.canvasSize = this.$el.height();
            this.snap = Snap(this.$svg[0]);
            this.arc = this.snap.path("");
            a = this.canvasSize / 2;
            this.snap.circle(a, a,
            (0.85 * this.canvasSize - this.config.ringStrokeWidth) / 2).attr({
                fill: "none",
                stroke: this.config.ringColor,
                strokeWidth: this.config.ringStrokeWidth
            });
            this.$el.data("apg", this);
            this.run();
            return this
        },
        run: function() {
            var a = this, c = this.canvasSize / 2, b = (0.85 * this.canvasSize - this.config.ringStrokeWidth) / 2 + 2, d = c - b, g = "";
            Snap.animate(0, 360 * this.percentage, function(f) {
                a.arc.remove();
                var e = Math.PI * (f-90) / 180, k = c + b * Math.cos(e), e = c + b * Math.sin(e), h = f / 360 * a.config.displayFactor;
                g = "M" + c + "," + d + " A" + b + "," + b + " 0 " + (180 < f ?
                1 : 0) + ",1 " + k + "," + e;
                a.arc = a.snap.path(g);
                a.arc.attr({
                    fill: "none",
                    stroke: a.config.arcColor,
                    strokeWidth: a.config.arcStrokeWidth
                });  
                //a.$label.html((a.config.originalValueWasInt ? Math.round(h) : h.toFixed(1)) + (a.config.originalWasPercentage ? '<span class="precent-sign">%</span>' : ""))
                // Hack to remove the comma values since our numbers do not have commas in them
                a.$label.html(Math.round(h)+ (a.config.originalWasPercentage ? '<span class="precent-sign">%</span>' : ""))
            }, 2E3, mina.easeinout)
        }
    };
    d.defaults = d.prototype.defaults;
    b.fn.animatedPercentageGraph = function(a) {
...
Base.extend = function(d, a) {
    var b = Base.prototype.extend;
    Base._prototyping=!0;
    var c = new this;
    b.call(c, d);
    c.base = function() {};
    delete Base._prototyping;
    var f = c.constructor, e = c.constructor = function() {
        if (!Base._prototyping)
            if (this._constructing || this.constructor == e)
                this._constructing=!0, f.apply(this, arguments), delete this._constructing;
        else if (null !== arguments[0])
            return (arguments[0].extend || b).call(arguments[0], c)
    };
    e.ancestor = this;
...
Base.prototype = {
    extend: function(d, a) {
        if (1 < arguments.length) {
            var b = this[d];
            if (b && "function" == typeof a && (!b.valueOf || b.valueOf() != a.valueOf()) && /\bbase\b/.test(a)) {
                var c = a.valueOf();
                a = function() {
            ...
            Base._prototyping || "function" == typeof this || (f = this.extend || f);
            for (var e = {
                toSource: null
            },
            h = ["constructor", "toString", "valueOf"], l = Base._prototyping ? 0 : 1; g = h[l++];)
                d[g] != e[g] && f.call(this, g, d[g]);
                for (var g in d)
                    e[g] || f.call(this, g, d[g])
        }
        return this
    }
};
var Base = Base.extend({
    ...
    forEach: function(d, a, b) {
        for (var c in d)
            void 0 === this.prototype[c] && a.call(b, d[c], c, d)
    },
    implement: function() {
        for (var d = 0; d < arguments.length; d++)
            if ("function" == typeof arguments[d])
                arguments[d](this.prototype);
            else 
                this.prototype.extend(arguments[d]);
        return this
    },
    toString: function() {
...
(function(d) {
    ...
    FlipClock.Base = Base.extend({
        ...
        constructor: function(a, b) {
            "object" !== typeof a && (a = {});
            "object" !== typeof b && (b = {});
            this.setOptions(d.extend(!0, {}, a, b))
        },
        callback: function(a) {
    ...
    FlipClock.Factory = FlipClock.Base.extend({
        ...
        constructor: function(a, b, c) {
            ...
            this.running=!1;
            this.base(c);
            this.$wrapper = d(a).addClass(this.classes.wrapper);
            this.time = new FlipClock.Time(this, b ? Math.round(b) : 0);
            this.timer = new FlipClock.Timer(this, c);
            this.lang = this.loadLanguage(this.language);
            this.face = this.loadClockFace(this.clockFace, c);
            this.autoStart && this.start()
        },
        loadClockFace: function(a, b) {
            var c;
            a = a.ucfirst() + "Face";
            c = FlipClock[a] ? new FlipClock[a](this, b) : new FlipClock[this.defaultClockFace + "Face"](this, b);
            c.build();
            return c
        },
        loadLanguage: function(a) {
            return FlipClock.Lang[a.ucfirst()] ? FlipClock.Lang[a.ucfirst()] : FlipClock.Lang[a] ? FlipClock.Lang[a] : FlipClock.Lang[this.defaultLanguage]
        },
        ...
        start: function(a) {
            var b = this;
            b.running || b.countdown &&
            !(b.countdown && 0 < b.time.time) ? b.log("Trying to start timer when countdown already at 0") : (b.face.start(b.time), b.timer.start(function() {
                b.flip();
                "function" === typeof a && a()
            }))
        },
    ...
    FlipClock.Face = FlipClock.Base.extend({
        ...
        build: function() {},
        createDivider: function(a, b, c, f) {
            "boolean" != typeof b && b || (c = b, b = a);
            "undefined" === typeof f && (f=!1);
            var e = "", e = c ? "": f ? '<span class="' + this.factory.classes.comma + '">,</span>': ['<span class="' + this.factory.classes.dot + ' top"></span>', '<span class="' + this.factory.classes.dot + ' bottom"></span>'].join("");
            a = this.factory.localize(a);
            a = ['<span class="' + this.factory.classes.divider + " " + (b ? b : "").toLowerCase() + '">', '<span class="' + this.factory.classes.label + '">' + (a ? a : "") + "</span>", e, "</span>"];
            return d(a.join(""))
        },
        createList: function(a, b) {
            "object" === typeof a && (b = a, a = 0);
            return new FlipClock.List(this.factory, a, b)
        },
        reset: function() {},
        setTime: function(a) {
            this.flip(a)
        ...
        addDigit: function(a) {
            ...
            a.$obj.insertBefore(this.factory.lists[0].$obj);
            this.factory.lists.unshift(a)
        },
        start: function() {},
        stop: function() {},
        flip: function(a, b) {
            var c = this;
            b || (c.factory.countdown ? (0 >= c.factory.time.time && c.factory.stop(), c.factory.time.time--) : c.factory.time.time++);
            var f = c.factory.lists.length - a.length;
            0 > f && (f = 0);
            d.each(a, function(a, d) {
                a += f;
                var e = c.factory.lists[a];
                if (e) {
                    var k = e.digit;
                    e.select(d);
                    d == k || b || e.play()
            ...
            for (var e = 0; e < a.length; e++)
                e >= f && c.factory.lists[e].digit != a[e] && c.factory.lists[e].select(a[e])
        }
    });
    FlipClock.List = FlipClock.Base.extend({
        ...
        select: function(a) {
            ...
            a = this.$obj.find('[data-digit="' + a + '"]');
            this.$obj.find("." + this.classes.active).removeClass(this.classes.active);
            this.$obj.find("." + this.classes.before).removeClass(this.classes.before);
            this.factory.countdown ? a.is(":last-child") ? this.$obj.find(":first-child").addClass(this.classes.before) : a.next().addClass(this.classes.before) : a.is(":first-child") ? this.$obj.find(":last-child").addClass(this.classes.before) : a.prev().addClass(this.classes.before);
            a.addClass(this.classes.active)
        },
        play: function() {
        ...
        createList: function() {
            for (var a =
            d('<ul class="' + this.classes.flip + " " + (this.factory.running ? this.factory.classes.play : "") + '" />'), b = 0; 10 > b; b++) {
                var c = d(['<li data-digit="' + b + '">', '<a href="#"><div class="up"><div class="shadow"></div>', '<div class="inn">' + b + "</div>", '</div><div class="down"><div class="shadow"></div>', '<div class="inn">' + b + "</div>", "</div></a></li>"].join(""));
                this.items.push(c);
                a.append(c)
            }
            return a
    ...
    FlipClock.Time = FlipClock.Base.extend({
        ...
        digitize: function(a) {
            ...
            b.length > this.minimumDigits && (this.minimumDigits = b.length);
            this.minimumDigits > b.length && b.unshift("0");
            return b
        },
        getDayCounter: function(a) {
            var b =
            [this.getDays(), this.getHours(!0), this.getMinutes(!0)];
            a && b.push(this.getSeconds(!0));
            return this.digitize(b)
        },
        getDays: function(a) {
        ...
        getMilitaryTime: function() {
            var a = new Date;
            return this.digitize([a.getHours(), a.getMinutes(),
            a.getSeconds()])
        },
        ...
        getTime: function() {
            ...
            return this.digitize([12 < b ? b-12: 0 === b ? 12: b, a.getMinutes(), a.getSeconds()])
        },
        getWeeks: function() {
        ...
        removeLeadingZeros: function(a,
        b) {
            var c = 0, f = [];
            d.each(b, function(d, h) {
                d < a ? c += parseInt(b[d], 10) : f.push(b[d])
            });
            return 0 === c ? f : b
        },
        toString: function() {
    ...
    d.fn.FlipClock = function(a, b) {
        "object" == typeof a && (b = a, a = 0);
        return new FlipClock(d(this), a, b)
    };
    d.fn.flipClock = function(a, b) {
        return d.fn.FlipClock(a, b)
    }
})(jQuery);
...
(function(d) {
    FlipClock.TwentyFourHourClockFace = FlipClock.Face.extend({
        ...
        build: function(a) {
            var b = this, c = this.factory.$wrapper.find("ul");
            a = a ? a : this.factory.time.time || this.factory.time.getMilitaryTime();
            a.length > c.length && d.each(a, function(a, c) {
                b.factory.lists.push(b.createList(c))
            });
            this.dividers.push(this.createDivider());
            this.dividers.push(this.createDivider());
            d(this.dividers[0]).insertBefore(this.factory.lists[this.factory.lists.length-2].$obj);
            d(this.dividers[1]).insertBefore(this.factory.lists[this.factory.lists.length-4].$obj);
        ...
        _clearExcessDigits: function() {
            for (var a = this.factory.lists[this.factory.lists.length-2], b = this.factory.lists[this.factory.lists.length-4], c = 6; 10 > c; c++)
                a.$obj.find("li:last-child").remove(), b.$obj.find("li:last-child").remove()
        }
    })
})(jQuery);
...
(function(d) {
    FlipClock.CounterFace = FlipClock.Face.extend({
        ...
        build: function() {
            var a = this, b = this.factory.$wrapper.find("ul"), c = [], f = this.factory.getTime().digitize([this.factory.getTime().time]);
            if (f.length > b.length && (d.each(f, function(b, d) {
                var f = a.createList(d);
                f.select(d);
                c.push(f)
            }), this.showCommas && (b = f.toString().replace(/,/g, "").replace(/\B(?=(\d{3})+(?!\d))/g, ","), 3 < c.length)
                ))for (f = 0; f < b.length; f++)
                "," === b[f] && (3 < f ? d(this.createDivider("", "", !1, "comma")).insertBefore(c[f - Math.floor(Math.pow(f, 1 / 3))].$obj) : d(this.createDivider("", "", !1, "comma")).insertBefore(c[f].$obj));
            d.each(c, function(a, b) {
                b.play()
            });
            this.factory.lists = c
        },
        flip: function(a) {
...
(function(d) {
    FlipClock.DailyCounterFace = FlipClock.Face.extend({
        ...
        build: function(a, b) {
            var c = this, f = this.factory.$wrapper.find("ul"), e = [], h = 0;
            b = b ? b : this.factory.time.getDayCounter(this.showSeconds);
            b.length > f.length && d.each(b, function(a, b) {
                e.push(c.createList(b))
            });
            this.factory.lists = e;
            this.showSeconds ? d(this.createDivider("Seconds")).insertBefore(this.factory.lists[this.factory.lists.length-2].$obj) : h = 2;
            d(this.createDivider("Minutes")).insertBefore(this.factory.lists[this.factory.lists.length-4 +
            h].$obj);
            d(this.createDivider("Hours")).insertBefore(this.factory.lists[this.factory.lists.length-6 + h].$obj);
            d(this.createDivider("Days", !0)).insertBefore(this.factory.lists[0].$obj);
            this._clearExcessDigits();
            this.autoStart && this.start()
        },
        flip: function(a, b) {
            b || (b = this.factory.time.getDayCounter(this.showSeconds));
            this.base(b, a)
        },
        _clearExcessDigits: function() {
            for (var a = this.factory.lists[this.factory.lists.length-2], b = this.factory.lists[this.factory.lists.length-4], c = 6; 10 > c; c++)
                a.$obj.find("li:last-child").remove(),
...
(function(d) {
    FlipClock.HourlyCounterFace = FlipClock.Face.extend({
        ...
        build: function(a, b) {
            var c = this, f = this.factory.$wrapper.find("ul"), e = [];
            b = b ? b : this.factory.time.getHourCounter();
            b.length > f.length && d.each(b, function(a, b) {
                e.push(c.createList(b))
            });
            this.factory.lists = e;
            d(this.createDivider("Seconds")).insertBefore(this.factory.lists[this.factory.lists.length-2].$obj);
            d(this.createDivider("Minutes")).insertBefore(this.factory.lists[this.factory.lists.length-4].$obj);
            a || d(this.createDivider("Hours", !0)).insertBefore(this.factory.lists[0].$obj);
            this.clearExcessDigits && this._clearExcessDigits();
            this.autoStart && this.start()
        },
        flip: function(a, b) {
            b || (b = this.factory.time.getHourCounter());
            this.base(b, a)
        },
        _clearExcessDigits: function() {
            for (var a = this.factory.lists[this.factory.lists.length-2], b = this.factory.lists[this.factory.lists.length-4], c = 6; 10 > c; c++)
...
(function(d) {
    FlipClock.TwelveHourClockFace = FlipClock.TwentyFourHourClockFace.extend({
        ...
        meridiumText: "AM",
        build: function(a) {
            a = a ? a : this.factory.time.time ? this.factory.time.time : this.factory.time.getTime();
            this.base(a);
            this.meridiumText = this._isPM() ? "PM" : "AM";
            this.meridium = d(['<ul class="flip-clock-meridium"><li>', '<a href="#">' + this.meridiumText + "</a>", "</li></ul>"].join(""));
            this.meridium.insertAfter(this.factory.lists[this.factory.lists.length-1].$obj)
        },
        flip: function() {
            this.meridiumText !=
            this._getMeridium() && (this.meridiumText = this._getMeridium(), this.meridium.find("a").html(this.meridiumText));
